function fout = BFieldNoise_ONEPHASE( dur_dark, noisex, noisey, noisez, magx, magy, magz, zpol, B0fac, g, randseed )
  ...
  for trial = (1:500)
    ...
    [t_values, sol_values] = ode45(@(t, Pv) diff_eq(t, Pv, Bv0, t_n_range, g), t_n_range, [sqrt(1-zpol*zpol) 0. zpol], options );
    save_values = [ save_values; sol_values(end, 1) sol_values(end, 2) sol_values(end, 3) ];
  end
  filename = sprintf('Simulations/noiseallsim1_dark-%d_x-%d_y-%d_z-%d_magx-%d_magy-%d_magz-%d_zpol-%d_B0fac-%d_g-%d_sol.txt', dur_dark, noisex, noisey, noisez, magx, magy, magz, zpol, B0fac, g )
